...
class LocalLLMManager:
    ...
    async def achat(self, backend: str, model: str, messages: list, stream: bool = False, json_mode: bool = False, **kwargs):
        if backend not in self.backends:
            raise ValueError(f"Backend '{backend}' is not supported. Choose from {list(self.backends.keys())}")
        
        adapter = self.backends[backend]
        return await adapter.achat(model_name=model, messages=messages, stream=stream, json_mode=json_mode, **kwargs)
